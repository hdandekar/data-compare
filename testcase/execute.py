...
class Comparison(object):
    ...
    def compare_data(self, srcdf, tgtdf, index_col_name, id, **kwargs):
        ...
        old_df = srcdf
        new_df = tgtdf
        print("old_df is: {}:{}".format(old_df.memory_usage(
            index=True).sum(), sys.getsizeof(old_df)))
        print("new_df is: {}:{}".format(new_df.memory_usage(
            index=True).sum(), sys.getsizeof(new_df)))
