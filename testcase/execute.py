...
class Comparison(object):
    ...
    def diff_pd(self, old_df, new_df, idx_col):
        ...
        old_df = (
            pd.merge(old_df, common_rows, how="outer", indicator=True)
            .query('_merge=="left_only"')
            .drop("_merge", axis=1)
        )
        new_df = (
            pd.merge(new_df, common_rows, how="outer", indicator=True)
            .query('_merge=="left_only"')
            .drop("_merge", axis=1)
        )

        src_only = pd.merge(
            old_df, new_df, on=idx_col, how="outer", suffixes=("", "_y"), indicator=True
        )
        src_only = src_only[src_only["_merge"] == "left_only"][old_df.columns]
        ...
        tgt_only = pd.merge(
            new_df, old_df, on=idx_col, how="outer", suffixes=("", "_y"), indicator=True
        )
        tgt_only = tgt_only[tgt_only["_merge"] == "left_only"][new_df.columns]
        ...
        old_df = (
            pd.merge(old_df, src_only, how="outer", indicator=True)
            .query('_merge=="left_only"')
            .drop("_merge", axis=1)
        )
        logger.info("Completed keeping only the same data in old_df")
        new_df = (
            pd.merge(new_df, tgt_only, how="outer", indicator=True)
            .query('_merge=="left_only"')
            .drop("_merge", axis=1)
        )
        ...
        for sname, data in diff.items():
            if sname == "changed":
                data.reset_index().to_csv(
                    "{}_{}.csv".format(id, sname), index=False, header=True
                )
            else:
                data.to_csv("{}_{}.csv".format(id, sname), index=False, header=True)
    ...
    def get_data(self, dbtype, host, db, user, pwd, sql, warehouse_name, schema_name):
        ...
        if dbtype == "Snowflake":
            ...
            cnx = pymssql.connect(server=host, database=db, user=user, password=pwd)
            # cursor = cnx.cursor()
            df = psql.read_sql(sql, cnx)
            cnx.close()
            logger.info("Completed pulling data from {} database".format(db))
    ...
    def get_src_data(self, conn, sql):
        ...
        result_set = self.get_data(
            dbtype, host, db, user, pwd, sql, warehouse_name, schema_name
        )
        return result_set
